...
namespace InterpreterScripts.Funcslibrary.Library
{
    class VoidScripts
    {
        [FuncDescription("MsgBox(string title, object text1...) - выводит сообщение, объединяя все переданные аргументы.")]
        ...
        [FuncDescription("ShowHideDesktopIcons(bool show) - показывает/скрывает значки рабочего стола.")]
        ...
        [FuncDescription("Delay(int ms) - приостанавливает выполнение скрипта на ms миллисекунд.")]
        ...
        [FuncDescription("KillProcessesByName(string name1, string name2...) - находит процессы с именами name... и останавливает их.")]
        ...
        [FuncDescription("KillProcessesByID(int id1, int id2...) - находит процессы с ID id1... и останавливает их.")]
        ...
        [FuncDescription("ProcessRun(string path) - запускает процесс по адрессу path(ссылки, приложения, файлы, пути в проводнике).")]
        ...
        [FuncDescription("RunCmd(string command) - выполняет указанную команду в консоли cmd.")]
        ...
        [FuncDescription("Shutdown() - выключает компьютер.")]
        ...
        [FuncDescription("Restart() - перезагрузка компьютера.")]
        ...
        [FuncDescription("SendKeysWait(string Key) - отправляет активному окну кнопку/текст и ждёт окончания обработки.")]
        ...
        [FuncDescription("SetCursorPos(int x, int y) - перемещает курсор на заданные координаты.")]
        ...
        [FuncDescription("MoveCursor(int x, int y, int delay) - перемещает курсор на заданные координаты с промежутком между перемещениями delay.")]
        ...
        [FuncDescription("MouseEvent(string event) - имитарует событие мыши. Для получения доступных событий использовать GetMouseEventsHelp().")]
        ...
        [FuncDescription("SetClipboardText(string text) - вставляет строку text в буфер обмена Windows.")]
